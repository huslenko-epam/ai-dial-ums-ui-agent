...
class ConversationManager:
    ...
    async def create_conversation(self, title: str) -> dict:
        ...
        await self.redis.set(
            f"{CONVERSATION_PREFIX}{conversation_id}", json.dumps(conversation)
        )

        await self.redis.zadd(
            CONVERSATION_LIST_KEY, {conversation_id: datetime.now(UTC).timestamp()}
        )

        logger.info(
            "Conversation created",
            extra={"conversation_id": conversation_id, "title": conversation["title"]},
        )
    ...
    async def chat(
        self, user_message: Message, conversation_id: str, stream: bool = False
    ):
        """
        ...
        if not messages:
            messages.append(
                Message(
                    role=Role.SYSTEM, content=os.getenv("SYSTEM_PROMPT", SYSTEM_PROMPT)
                )
            )
    ...
    async def _non_stream_chat(
        ...
        message = await self.dial_client.response(messages)

        await self._save_conversation_messages(conversation_id, messages)

        return {"content": message.content or "", "conversation_id": conversation_id}
